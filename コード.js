...
function onOpen() {
  ...
  // ここでIDを控えておく。これを忘れるとウェブアプリ側が何も出来ない。
  try { getSS_(); } catch (e) {}
  // メニューは項目が増えすぎると探しづらくなるため、毎週使うものだけを第1階層に置き、
  ...
  ui.createMenu('名簿システム')
    ...
    .addSubMenu(ui.createMenu('🎯 抽選ルーレット')
      .addItem('ビジター招待数をH列に入れる', 'menuFillRouletteVisitorCounts')
    ...
    .addSubMenu(ui.createMenu('⚙️ 設定')
      ...
      .addSeparator()
      .addItem('休会日', 'openHolidayDialog')
      .addItem('メールテンプレート', 'openTemplateDialog')
      .addItem('割り振り表の特記事項', 'openAllocationNoteDialog')
...
function saveAllocationSheet(meetingDateVal, displayVal, visitors, pool, facilAlloc, orienAlloc, roomAlloc, connectReq, mergedWith) {
  ...
  // 列幅(px)。A4縦・余白0.25インチの印字幅(約197mm=746px)に収まるようにしてある。
