#!/usr/bin/env python3
"""後半スライドの「出力」を、差し込み口付きのテンプレートに作り変える。

毎週手で書き換えている箇所（推薦のことば、抽選コーナー）に
{{ }} を入れる。デザイン・座標・フォント・画像には触らない。

    python3 tools/build_meeting_second_template.py <出力.pptx> <テンプレート.pptx>
"""
import os
import shutil
import subprocess
...
def main():
    ...
    try:
        ...
        r = subprocess.run(['node', 'tools/make_meeting_second_template.js', work], cwd=ROOT)
        if r.returncode != 0:
